...
public class JsonModel extends Model {
	...
	public boolean renderNormal(String name, BufferBuilder bb) {
		...
		l.forEach(e -> {
			...
			GlStateManager.translate(e.rotOrigin.x, e.rotOrigin.y, e.rotOrigin.z);
			GlStateManager.rotate((float) e.angle, e.axis == Axis.X ? 1 : 0, e.axis == Axis.Y ? 1 : 0, e.axis == Axis.Z ? 1 : 0);
			GlStateManager.translate(-16, -8, -16);
