...
public class JsonModel extends Model {
	...
	public boolean renderNormal(String name, BufferBuilder bb) {
		...
		l.forEach(e -> {
			...
			bb.begin(GL11.GL_QUADS, VertexFormats.POSITION_UV);
			...
			e.faces.entrySet().forEach(fe -> {
				...
				for(int i = 0;i<4;i++){
					...
					bb.
					vertex(    v.x * size.x + e.from.x,
							v.y * size.y + e.from.y,
							v.z * size.z + e.from.z).
					texture(    (t.x * us + f.u.x) / 16,
							(t.y * vs + f.u.y) / 16).
					/*tex(    t.x,
							t.y).*/
	...
	public static class Element {
		...
		public Element(Map<String, Object> json) {
			...
			if(json.containsKey("rotation")) {
				...
				axis = Axis.fromName((String) r.get("axis"));
	...
	private static final Vec2f OI = new Vec2f(0, 1);
	private static final Vec2f IO = new Vec2f(1, 0);
	private static final Vec2f II = new Vec2f(1, 1);
